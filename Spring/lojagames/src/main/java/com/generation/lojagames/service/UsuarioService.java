...
public class UsuarioService {
	...
	public Optional<Usuario> cadastrarUsuario(Usuario usuario) {
		...
		if (idade < 18) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "NÃO PERMITIDO USUÁRIO MENOR DE 18 ANOS!", null);
		}
	...
	public Optional<Usuario> atualizarUsuario(Usuario usuario) {
		...
		if (usuarioRepository.findByUsuario(usuario.getUsuario()).isPresent()) {
			...
			if (idade < 18) {
				throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "NÃO PERMITIDO USUÁRIO MENOR DE 18 ANOS!",
						null);
			}
